...
**キー設定:**
- **Merge All Meshes** — エクスポート時に 1 つのメッシュに統合 (推奨)
- **Apply Shape Keys to Basis** — エクスポート前にすべてのアクティブなシェイプキーをベースメッシュ形状にマージ
- **Include Armature / Mesh / Materials / Animations** — エクスポートに含めるデータをトグル
...
**何をするか:** VRM エクスポート要件に対してシーンを検証します。必要な humanoid ボーン、メタデータの完全性、マテリアルセットアップをチェックします。

**Run VRM Lint** をクリックして結果を確認します。
...
**ターゲットオプション:**
- **VRChat (Unity FBX)** — 標準 VRChat エクスポート
- **VRM** — VRM エクスポーターに委任
- **MMD (PMX)** — MMD エクスポーターに委任
- **Unreal Engine FBX** — Unreal 固有の設定と LOD サポート付きの FBX
- **Unity General** — FBX + Unity プロジェクト用メタデータサイドカー

**一般的な FBX 設定:**
- **Include Armature / Mesh / Materials / Animations** — エクスポートコンポーネントをトグル
- **Bake Animation** — 互換性のために制約駆動アニメーションを生キーフレームに変換
- **FBX Version** — ASCII またはバイナリ形式
